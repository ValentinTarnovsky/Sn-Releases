...
- Any number of currencies, all declared in `config.yml`. The plugin links no economy plugin of its own: a currency either drives give/take console commands with a balance placeholder, or names an EdTools currency id and uses that API directly.
...
- A payout never travels through a path that fires the economy plugin's own credit event, so server boosters, money enchants and pets cannot multiply a prize.
...
- **PlaceholderAPI**: needed by every command-backed currency, because its balance is read through a placeholder, and needed by the eight placeholders SnCoinFlip provides. Without it a command-backed currency still registers, but every balance reads as zero and every wager on it is refused.
...
Both are soft dependencies, so the plugin starts without them. The shipped currency set does not work without them.
