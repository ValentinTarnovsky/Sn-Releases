...
The `xp-info` tile of `main.yml` binds `{multiplier}` (the effective XP per block for a tool without an override) and one `{multiplier_<tool>}` per tool listed under `xp.passive.tool-overrides` - these are **per-block XP amounts**, rate times boost, not the boost itself, so `0.5` on a server paying half an XP per block means no boost is active. Since **2.4.0** the boost itself is bound beside them: `{boost}` is the pet boost in percentage points at one decimal (`48.0`) and `{boost_factor}` the same as a multiplier (`1.48`), the exact numbers `%battlepass_boost%` and `%battlepass_boost_factor%` return. The shipped tile prints `Pet boost: +{boost}%`; a server updating from 2.3.0 keeps its own lore, so add the line by hand.
