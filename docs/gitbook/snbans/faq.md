...
### How do I change the length of a punishment that is already active?
Issue the new one over it. With `snbans.overwrite`, `/ban Notch 1d griefing` on a player who is already banned for four minutes replaces the old ban: the new one is the only one enforced, on every server, and the old one stays in `/history` as lifted by you. The same works for `/mute` and `/blacklist`. Without the node you get "already has an active ban" and have to `/unban` first, which is the default so that a rank holding `snbans.ban` cannot shorten a permanent ban to one second. The console always gets that refusal, node or not, so an anticheat or chat filter punishing through it can never replace a sanction that is already in force - lift it first when you work from the console.

### Do colour codes in my lang words show up in Discord?
No. The words SnBans takes from the lang file - the console name in `messages.format.console`, the `{status}` and `{type}` words, "Permanent" - are posted to Discord as plain text, so `&#FF0000WatchDog` renders red in chat and as `WatchDog` in the embed. Your own markdown in `webhooks.yml` is left alone.
...
Check three things in this order. Every backend must use `database.type: mysql` against the **same** schema - read each backend's boot log, which prints `Database: MySQL, cross-server sync every Xs` when its poller is armed and `standalone install (no cross-server sync)` when it is not. `sync.interval-seconds` is the delay you should expect. And every backend must be on **1.8.2 or newer**: before that release the poller told a peer's row from its own by comparing `server-name`, so a network that had copied the plugin folder to each backend - leaving the shipped `"Server"` on all of them - had every peer row read as locally issued. Nothing propagated and nothing was logged. On a peer, an arriving punishment is enforced and announced under that server's own `broadcasts` rules, and so is an arriving `/unban`, `/unmute` or `/unblacklist`, with the `-s` or `-p` it was typed with. Each server announces it exactly once; the server that ran the command does not announce it a second time. That needs the server that ran the command to be on the current version as well, because the unban announcement travels through the `snbans_reverts` table, which older builds do not write.

If a mute is announced only on the server the command was run on, and the player keeps talking elsewhere until they reconnect, that is exactly the pre-1.8.2 symptom. Updating fixes it with no configuration change.
