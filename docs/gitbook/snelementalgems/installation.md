...
3. Start the server once. SnElementalGems creates a license file at `plugins/SnElementalGems/license.yml`.
...
{% hint style="info" %}
SnElementalGems is licensed. The key lives in `plugins/SnElementalGems/license.yml`. Without a valid key the plugin refuses to enable.
{% endhint %}
