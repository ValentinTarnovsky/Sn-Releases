...
Create it with `/mg admin tntrun create <name>`. Get the cuboid wand with `/mg admin wand region`, left-click one corner of the arena and right-click the opposite one (the edges are drawn with particles while you select), then run `/mg admin tntrun setregion <map>`. Stand on the top layer and run `setstart <map>`, stand in the mini-lobby and run `setwaiting <map>`, and set the elimination height with `setelimy <map> <y>`. `/mg admin tntrun help` lists everything else (`setdelay`, `setwinners`, `settimelimit`, `setend`).
