...
### Can a player survive by sneaking onto the edge of a block?
No. A player is 0.6 blocks wide, so the block their body rests on is not always the one under their feet: sneaking at the rim of a hole lets the hitbox hang over the gap while a sliver still sits on the neighbouring block. Every block column the hitbox overlaps is scheduled for removal, not just the one under the player's centre, so a corner is no safer than the middle of a block.
